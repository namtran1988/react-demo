...
export function Register(){
    ...
    const onSubmitHandler = useCallback(async(e: React.FormEvent<HTMLFormElement>) => { 
        e.preventDefault();
        await axios.postForm(`${API_URL}/account/register`,formData).then(e=>{console.log("test response", e)})
    }, [formData]);
    return (<form onSubmit={onSubmitHandler}>
        <div><label>Email: </label>
            <input type="text" onChange={onInputChange} name="email" autoComplete="false" /></div>
